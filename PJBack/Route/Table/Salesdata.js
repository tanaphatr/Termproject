...
router.put('/:sales_data_id', async (req, res) => {
    ...
    try {
        ...
        const result = await db.query(
            'UPDATE Salesdata SET sale_date = ?, sales_amount = ?, profit_amount = ?, event = ?, day_of_week = ?, festival = ?, weather = ?, Temperature = ?, Back_to_School_Period = ?, Seasonal = ? WHERE sales_data_id = ?',
            [sale_date, sales_amount, profit_amount, event, day_of_week, festival, weather, Temperature, Back_to_School_Period, Seasonal, sales_data_id]
        );
        ...
        res.status(200).json({ message: "Record updated successfully", sales_data_id, sale_date, sales_amount, profit_amount, event, day_of_week, festival, weather, Temperature, Back_to_School_Period, Seasonal });
    } catch (err) {
